...
/*
 * This structure has the shared EVR data for a particular open file.
 */
struct shared_mrf {
    struct mrf_dev *parent;      /* Set if in use, otherwise NULL */
...
struct mrf_dev {
  ...
  int    refcount_ev;        /* FPGA memory mapped register map reference
				count. We allow several simultaneous connections
